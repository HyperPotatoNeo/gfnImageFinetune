...
def parse_args():
  ...
  parser.add_argument(
      "--buffer_size", type=int, default=500, help="size of replay buffer"
  )
  parser.add_argument(
...
def _update_output_dir(args):
  ...
  coeff_log = "_re" + str(args.reward_weight)
  ...
  for _ in range(args.g_step):
    ...
    with torch.no_grad():
      ...
      for i in range(len(latents_list) - 1):
        ...
        state_dict["old_log_prob"] = torch.cat(
...
def _train_policy_func(
  ...
  with torch.no_grad():
    ...
    indices = img_idx + (state_dict["state"].shape[0]//50)*sample_indices
    batch_state = state_dict["state"][indices]
    ...
    log_pf_prior = (state_dict["old_log_prob"][img_idx + (state_dict["state"].shape[0]//50)*np.arange(50)]).cuda()
    log_pf_prior_sum = log_pf_prior.sum()
    log_pf_post = (state_dict["log_prob"][img_idx + (state_dict["state"].shape[0]//50)*np.arange(50)]).cuda()
    log_pf_post_sum = log_pf_post.sum()
  logZ = logC.repeat(args.p_batch_size).reshape([args.p_batch_size, 1])
  with torch.no_grad():  
    ...
    # white noise baseline
        adv = adv - adv.mean()
  loss = (log_pf_post/args.reward_weight + logZ - log_pf_prior/args.reward_weight - adv)**2
  loss = loss.mean()
...
def main():
  ...
  logC = torch.nn.Parameter(torch.tensor(0.).to('cuda'))
  optimizer = optimizer_cls(
      [{'params': lora_layers.parameters()}, {'params': [logC], 'lr': args.c_learning_rate}],
  ...
  state_dict["old_log_prob"] = _map_cpu(torch.FloatTensor().to(weight_dtype))
